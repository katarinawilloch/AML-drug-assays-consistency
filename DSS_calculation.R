...
length(unique(enserink_breeze_input$patient_id))
...
unique(enserink_breeze_input$dose)
...
length(unique(fimm_raw_response$drug))
unique(fimm_raw_response$Final.Conc)
...
calc_dss(df, 'drug', 'Final.Conc', 'sample', 'doseResponses', '~/Desktop/UiO/Project 1/Data/Initial cleansing/dss_github_fimm_full_drug_set_v1.csv', graph = TRUE, viability = TRUE, set_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/FIMM")
...
length(unique(beat_aml_raw_response$dbgap_subject_id))
...
unique(beat_aml_raw_response$well_concentration)
