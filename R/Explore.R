...
beat_aml_karolinska_FIMM_enserink_for_heatmap_AUC <- pivot_wider(beat_aml_karolinska_FIMM_enserink_AUC, names_from = drug, values_from = AUC)

ppca_AUC <- pca(beat_aml_karolinska_FIMM_enserink_for_heatmap_AUC[,-c(1, 2)], method="ppca", nPcs=3, seed=123)
## Get the estimated complete observations
ppca_scores_AUC <- scores(ppca_AUC)
nrow(ppca_scores_AUC)
ppca_data_AUC <- merge(ppca_scores_AUC, beat_aml_karolinska_FIMM_enserink_for_heatmap_AUC, by = 'row.names')
library(ggrepel)
library(ggnewscale)
ppca_data_AUC$lab <- gsub('BeatAML', 'Beat AML', ppca_data_AUC$lab)
ppca_data_AUC$lab <- factor(ppca_data_AUC$lab, levels = c("Beat AML", "Oslo", "Helsinki", "Karolinska"))
beat_aml_karolinska_FIMM_enserink_avg_AUC <- beat_aml_karolinska_FIMM_enserink_AUC %>% group_by(Patient.num) %>% dplyr::summarize('Average AUC' = mean(AUC))
ppca_data_AUC <- ppca_data_AUC %>% left_join(beat_aml_karolinska_FIMM_enserink_avg_AUC, by = 'Patient.num')
ppca_data_AUC$`Average AUC` <- as.numeric(ppca_data_AUC$`Average AUC`)
ppca_data_AUC$Lab <- as.factor(ppca_data_AUC$lab)
ppca_plot <- ggplot(as.data.frame(ppca_data_AUC), aes(x = PC1, y = PC2, color=Lab)) +
  geom_point() +
  #geom_text_repel(data = furthest_points, aes(label = Patient.num), size = 4, box.padding = 0.5, max.overlaps = 20) +
  #scale_color_gradient(low = "lightblue", high = "blue") + 
  scale_color_manual(values = c("Beat AML"="#8dd3c7", "Oslo"="#fdb462", "Helsinki"= "#fb8072","Karolinska"="#80b1d3")) +
