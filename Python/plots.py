...
df = from_contents({'Enserink_lab': df_enserink_drugs['pubchem_drug_name'], 'BeatAML': beat_aml_inhibitor['pubchem_drug_name'], 'FIMM': fimm_drug_library['pubchem_drug_name'], 'Karolinska': karolinska_drug_library['pubchem_drug_name']})
print(df)


upset = UpSet(df, show_counts="%d", show_percentages=True)
upset.style_subsets(present=["BeatAML", "Enserink_lab", "FIMM", "Karolinska"], facecolor="blue")
upset.plot()
