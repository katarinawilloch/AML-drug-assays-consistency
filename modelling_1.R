...
unique(experimental_var$cell_counting_method)
...
names(all_response_metrics)
beatAML_t <- subset(all_response_metrics, lab == 'BeatAML', select = c(Patient.num, Disease.status, Tissue, lab, sample))
beatAML_t <- unique(beatAML_t[order(beatAML_t$Patient.num),])

oslo_t <- subset(all_response_metrics, lab == 'Oslo', select = c(Patient.num, Patient_ID, Disease.status, Tissue, lab, sample))
oslo_t <- unique(oslo_t[order(oslo_t$Patient.num),])

karolinska_t <- subset(all_response_metrics, lab == 'Karolinska', select = c(Patient.num, Disease.status, Tissue, lab, sample))
karolinska_t <- unique(karolinska_t[order(karolinska_t$Patient.num),])

helsinki_t <- subset(all_response_metrics, lab == 'Helsinki', select = c(Patient.num, Disease.status, Tissue, lab, sample))
helsinki_t <- unique(helsinki_t[order(helsinki_t$Patient.num),])


summary_table <- all_response_metrics %>% group_by(lab, sample) %>% summarize(count = n_distinct(Patient_ID))

write.xlsx(summary_table, "/Users/katarinawilloch/Desktop/UiO/Project 1/summary_table.xlsx", rowNames = FALSE)

all_response_metrics$Patient_ID <- all_response_metrics$Patient.num
all_response_metrics$Patient.num <- as.character(all_response_metrics$Patient.num)
all_response_metrics <- all_response_metrics %>% dplyr::mutate(Patient.num = ifelse(lab == 'Oslo' | lab == "Beat AML", gsub("_.*", "", Patient.num), Patient.num)) %>%
  mutate(Patient.num = ifelse(lab == 'Helsinki', str_extract(Patient.num, "^[^_]+_[^_]+"), Patient.num)) %>%
  mutate(Patient_ID = ifelse(lab == 'Karolinska', paste0(Patient.num, '_', Disease.status, '_', Tissue,'_', sample), Patient_ID))
...
skewness(all_response_metrics$DSS2_boxcox_sclaed2)
kurtosis(all_response_metrics$DSS2_boxcox_sclaed2)
sd(all_response_metrics$DSS2_boxcox_sclaed2)
...
print(g)
g[[1]]$labels$subtitle
ggsave("Desktop/UiO/Project 1/Figures/draw/Difference_DSS2_per_drug.png", plot = g, width = 50, height = 60, units = "cm", limitsize = FALSE)
...
for (i in 1:4) {
  if(!is.null(g[[i]]$labels$subtitle)){
    ...
    # Extract only "Kruskal-Wallis: italic(p) == value"
    subtitle_mod <- str_replace(subtitle_str, 'list\\(chi\\["Kruskal-Wallis"\\]\\^2.*?italic\\(p\\) ==      "', "p-value = ")
    subtitle_mod <- str_replace(subtitle_mod, '", widehat\\(epsilon\\)\\["ordinal"\\]\\^2.*', "")
...
mixed_model_time_until_sample_usage_box_cox_scaled_re_intercept <- lmer(DSS2_boxcox_sclaed2 ~ time_until_sample_usage + (1|Patient.num) + (time_until_sample_usage|drug), all_response_metrics, REML = TRUE)
test1 <-model_diagnostics(mixed_model_time_until_sample_usage_box_cox_scaled_re_intercept, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/DSS2/time_until_sample_usage/", check_model = FALSE)

random_effects <- ranef(mixed_model_time_until_sample_usage_box_cox_scaled_re_intercept)
...
for(n in names(random_slopes)){
  ...
  if(n != "(Intercept)"){
    ...
    print(shapiro_slope)
    print(ad.test(random_slopes[,n]))
    print(cvm.test(random_slopes[,n]))
  }
}

shapiro_slope$p.value

diagnostics <- check_model(mixed_model_time_until_sample_usage_box_cox_scaled_re_intercept)
...
check_heteroscedasticity(mixed_model_time_until_sample_usage_box_cox_scaled_re_intercept)
check_outliers(mixed_model_time_until_sample_usage_box_cox_scaled_re_intercept)
...
pp_check_plot <- check_model(medium_model, check = "pp_check", panel = FALSE, title_size = 10, axis_title_size = 10, base_size = 0, colors = c("#fccde5", "#80b1d3", "black"))
...
plot_pp_check_adjusted <- plot_pp_check$PP_CHECK + theme(text = element_text(family = "Arial", color= "black", size = 10),
                               ...
                               ) + labs(x = "BoxCox Transformed and Scaled DSS2")
plot_pp_check_adjusted
ggsave("/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/V3/pp_check.png", plot = plot_pp_check_adjusted,width = 9, height = 6.5, units="cm")
...
result <- check_homogeneity(medium_model)
plot(result)
...
plot_homogeneity_adjusted <- plot_homogeneity$HOMOGENEITY + theme(text = element_text(family = "Arial", color= "black", size = 10),
                                                      plot.title = element_text(hjust = 0.5, vjust = 1),
                                                      axis.title.x = element_text(family = "Arial", color = "black", size = 10),  
                                                      axis.title.y = element_text(family = "Arial", color = "black", size = 10),
                                                      axis.text.x = element_text(family = "Arial", color = "black", size = 8),  
...
ggsave("/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/V3/combo_plots2.png", plot = comboplot2, width = 20.76, height = 7.09, units="cm")
...
model_diagnostics(mixed_model_medium_box_cox_scaled_re_intercept, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/DSS2/medium/", check_model = TRUE)

####----Cells----
mixed_model_cells_box_cox_scaled_re_intercept <- lmer(DSS2_boxcox_sclaed2 ~ cells +(1|Patient.num) + (1 + cells|drug),all_response_metrics)
summary(mixed_model_cells_box_cox_scaled_re_intercept)
check_model(mixed_model_cells_box_cox_scaled_re_intercept)
model_diagnostics(mixed_model_cells_box_cox_scaled_re_intercept, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/DSS2/cells/", check_model = TRUE)


####----Microenv stimuli----
mixed_model_micro_env_stimuli_box_cox_scaled_re_intercept <- lmer(DSS2_boxcox_sclaed2 ~ microenvironmental_stimuli +(1|Patient.num) + (1 + microenvironmental_stimuli|drug),all_response_metrics)
...
model_diagnostics(mixed_model_micro_env_stimuli_box_cox_scaled_re_intercept, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/DSS2/microenv/", check_model = TRUE)



####----Cell counting method----
unique(all_datasets_v2$cell_counting_method)
mixed_model_cell_counting_method_box_cox_scaled_re_intercept <- lmer(DSS2_boxcox_sclaed2 ~ cell_counting_method + (1|Patient.num) + (1 + cell_counting_method|drug),all_response_metrics)
...
icc(mixed_model_cell_counting_method_box_cox_scaled_re_intercept)
subset(all_response_metrics, is.na(cell_counting_method))
model_diagnostics(mixed_model_cell_counting_method_box_cox_scaled_re_intercept, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/DSS2/cell_counting/", check_model = TRUE)


####----Positive Control + Sensitivity Readout + Nr of Concentration Points
mixed_model_sensitivity_readout_and_positive_control_box_cox_scaled_re_intercept <- lmer(DSS2_boxcox_sclaed2 ~ positive_control + (1|Patient.num) +  (1 + positive_control|drug),all_response_metrics)
...
icc(mixed_model_sensitivity_readout_and_positive_control_box_cox_scaled_re_intercept)
model_diagnostics(mixed_model_sensitivity_readout_and_positive_control_box_cox_scaled_re_intercept, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/DSS2/positive_control/", check_model = TRUE)
...
mixed_model_centrifugation_procedure_box_cox_scaled_re_intercept <- lmer(DSS2_boxcox_sclaed2 ~ centrifugation_procedure + (1|Patient.num) + (1 + centrifugation_procedure|drug),all_response_metrics)
...
model_diagnostics(mixed_model_centrifugation_procedure_box_cox_scaled_re_intercept, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/DSS2/centrifugation_procedure/", check_model = TRUE)


####----Plate Reader----
##----here----
mixed_model_plate_reader_box_cox_scaled_re_intercept <- lmer(DSS2_boxcox_sclaed2 ~ plate_reader + (1|Patient.num) + (1 + plate_reader|drug),all_response_metrics)
...
c <- subset(all_response_metrics, is.na(plate_reader))
model_diagnostics(mixed_model_plate_reader_box_cox_scaled_re_intercept, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/DSS2/plate_reader/", check_model = TRUE)


####----Table of crossed re random intercept----
re_intercept_models_pn <-c(mixed_model_time_until_sample_usage_box_cox_scaled_re_intercept, mixed_model_medium_box_cox_scaled_re_intercept, mixed_model_cells_box_cox_scaled_re_intercept, mixed_model_micro_env_stimuli_box_cox_scaled_re_intercept, mixed_model_cell_counting_method_box_cox_scaled_re_intercept, mixed_model_sensitivity_readout_and_positive_control_box_cox_scaled_re_intercept, mixed_model_centrifugation_procedure_box_cox_scaled_re_intercept, mixed_model_plate_reader_box_cox_scaled_re_intercept)
df_re_intercept_models_pn <- model_results(re_intercept_models_pn, data_frame_org = all_datasets_v2)
#saved_backup_df_re_intercept_models <- df_re_intercept_models
df_re_intercept_models <- saved_backup_df_re_intercept_models
...
df_re_intercept_models$`p-valueᶜ` <- round(df_re_intercept_models$`Corrected p Value`, 4)
df_re_intercept_models$`p-valueᶜ` <- format(df_re_intercept_models$`p-valueᶜ`, scientific = FALSE, trim = TRUE)
df_re_intercept_models <- df_re_intercept_models %>% mutate(`p-valueᶜ` = ifelse(df_re_intercept_models$`p-valueᶜ` == '0.0000', '<0.0001', as.character(df_re_intercept_models$`p-valueᶜ`)))
#df_re_intercept_models <- df_re_intercept_models %>% dplyr::rename(`p-valueᶜ` = `p-value*`)

#df_re_intercept_models$se <- (log(df_re_intercept_models$Upper) - log(df_re_intercept_models$`Fixed Effect Coefficient`))/1.96
p <- forest(df_re_intercept_models[,c('Experimental Variable', ' ', 'p-valueᶜ')],
            ...
            footnote = "\n\nᵃLymphoPrepTM gradient centrifugation \nᵇSupernatant isolation at 300g 10min; density centrifugation at 400g for 20min without brake\nᶜBonferroni corrected",
            theme = tm, 
            xlab = 'Change in Scaled DSS2',
            font.label = list(size = 14, family = "Arial"), #10
            font.ticks = list(size = 9), #9
            ) 

p1 <- as.ggplot(p) + theme_minimal() +  # Set your base_size here
  theme(
...
plot(p)
p_wh <- get_wh(p)
pdf('/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/draw/Forest_plot_DSS2.pdf',width = p_wh[1], height = p_wh[2])
plot(p)
dev.off()

library(patchwork)
library("ggplotify")
patchwork::wrap_ggplot_grob(p)
set <- p/(plot_influential_adjusted | plot_homogeneity_adjusted) 
grid.arrange(as.ggplot(p), plot_influential_adjusted, plot_homogeneity_adjusted)
print(as.ggplot(p)$layers)
p_scaled <- ggdraw() + draw_grob(p, x = 0.5, y = 0.5, width = 0.9, height = 0.9)
...
homogeneity_grob <- ggplotGrob(plot_homogeneity_adjusted)

# Extract widths and heights
plot_width <- sum(plot_grob$widths)
plot_height <- sum(plot_grob$heights)

homogeneity_width <- sum(homogeneity_grob$widths)

# Desired dimensions for the forestplot
forestplot_width <- plot_width + homogeneity_width
forestplot_height <- plot_height

scale_width <- forestplot_width / sum(scaled_p$widths)
scale_height <- forestplot_height / sum(scaled_p$heights)

# Adjust the forestplot dimensions
scaled_p$widths <- scaled_p$widths * scale_width
scaled_p$heights <- scaled_p$heights * scale_height

wrapped_forestplot <- patchwork::wrap_elements(grid::grobTree(scaled_p))
...
ggsave('/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/V3/comboplots.png', plot = final_plot, width = 20, height = 28.7)
...
dynamic_forestplot <- function(p, width_cm, height_cm, dpi = 1200, filename = "dynamic_forestplot.png") {
  ...
  # Save plot
  png(filename, width = width_in * dpi, height = height_in * dpi, res = dpi)
  grid.newpage()
  grid.draw(p)
  dev.off()
}
...
# Usage
dynamic_forestplot(p, width_cm = 20, height_cm = 12, filename = '/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/draw/Forest_plot_DSS2.png')


# Define the physical dimensions (cm) and resolution
output_width_cm <- 16 # 14.5
output_height_cm <- 10 #10
...
png('/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/draw/Forest_plot_DSS2.png', height = 12, width = 28, unit = "cm", res = 300) #height = 10, width = 24.8
grid.newpage()
grid.draw(scaled_p)
dev.off()

ggsave('/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/draw/Forest_plot_DSS2.png', plot = scaled_p, unit = "cm") #height = 6.5
...
skewness(all_response_metrics$AUC_boxcox_sclaed2)
kurtosis(all_response_metrics$AUC_boxcox_sclaed2)
sd(all_response_metrics$AUC_boxcox_sclaed2)
...
model_diagnostics(mixed_model_time_until_sample_usage_auc, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/AUC/time_until_sample_usage/", check_model = TRUE)
...
model_diagnostics(mixed_model_medium_auc, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/AUC/medium/", check_model = TRUE)
...
model_diagnostics(mixed_model_cells_auc, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/AUC/cells/", check_model = TRUE)
...
model_diagnostics(mixed_model_micro_env_stimuli_auc, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/AUC/microenvironmental_stimuli/", check_model = TRUE)
...
model_diagnostics(mixed_model_cell_counting_method_auc, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/AUC/cell_counting_method/", check_model = TRUE)
...
model_diagnostics(mixed_model_sensitivity_readout_and_positive_control_auc, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/AUC/positive_control/", check_model = TRUE)
...
model_diagnostics(mixed_model_centrifugation_procedure_auc, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/AUC/centrifugation_procedure/", check_model = TRUE)
...
model_diagnostics(mixed_model_plate_reader_auc, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/AUC/plate_reader/", check_model = TRUE)
...
#all_response_metrics$IC50_boxcox <- log(all_response_metrics$IC50/(1-all_response_metrics$IC50))
...
model_diagnostics(mixed_model_time_until_sample_usage_ic50, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/IC50/time_until_sample_usage/", check_model = TRUE)
...
model_diagnostics(mixed_model_medium_ic50, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/IC50/medium/", check_model = TRUE)
...
model_diagnostics(mixed_model_cells_ic50, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/IC50/cells/", check_model = TRUE)
...
model_diagnostics(mixed_model_micro_env_stimuli_ic50, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/IC50/microenvironmental_stimuli/", check_model = TRUE)
...
model_diagnostics(mixed_model_cell_counting_method_ic50, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/IC50/cell_counting_method/", check_model = TRUE)
...
model_diagnostics(mixed_model_sensitivity_readout_and_positive_control_ic50, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/IC50/positive_control/", check_model = TRUE)
...
model_diagnostics(mixed_model_centrifugation_procedure_ic50, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/IC50/centrifugation_procedure/", check_model = TRUE)
...
model_diagnostics(mixed_model_plate_reader_ic50, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/IC50/plate_reader/", check_model = TRUE)
...
model_diagnostics(mixed_model_time_until_sample_usage_dss1, output_dir = "/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/Diagnostics/IC50/plate_reader/", check_model = TRUE)
...
df <- df %>% mutate(`Experimental Variable`= case_when(`Fixed Effect Term` == "Positive Control: BzCl + Nr of Concentration Points: 5 + Drug Sensitivity Readout: CellTiter-Glo" ~ "Positive Control & Dose Points & Readout", 
                                                                                               ...
                                                                                               `Fixed Effect Term` ==  "microenvironmental_stimuliNone" ~ "Microenvironmental Stimuli: None",
                                                                                               `Fixed Effect Term` ==  "Microenvironmental stimuli: Transiently cultured with feeder cells, activate samples with autologous BM T helper cells in the presence of IL-2 and a T-cell expansion cocktai" ~ "Microenvironmental Stimuli: Yes",
                                                                                               `Fixed Effect Term` ==  "Plate readers: Pherastar, Cytation5, Insight, Tecan" ~ "Plate Reader: Pherastar",
                                                                                               `Fixed Effect Term` ==  "plate_readerVictorX" ~ "Plate Reader: VictorX",
...
p <- forest(df_re_intercept_models[,c('Experimental Variable', ' ', 'p-value*')],
            ...
            xlim = c(-1, 1),
            #ticks_at = c(-1, -0.5, 0, 0.5, 1),
            footnote = "\n\n*1:LymphoPrepTM gradient centrifugation \n*2: Supernatant isolation at 300g 10min; density centrifugation at 400g for 20min without brake\n*: Bonferroni corrected",
            theme = tm, 
            xlab = 'Change in Scaled DSS2',
...
category_colors <- c("#8dd3c7", "#ffffb3", "#bebada", "#fb8072", "#fdb462", "#b3de69")
...
tm <- forest_theme(
  ...
  footnote_gp = gpar(col = "black", cex = 0.6, fontsize = 8), 
  footnote_parse = FALSE, 
  legend_name = "   ",#Response Metric
  legend_position = "bottom",
  legend_value = c(" IC50", " DSS3", " DSS2", " DSS1", " rAUC", " AUC"), 
  legend_gp = gpar(fontsize = 10, fontfamily = "Arial", cex = 1),
...
df_wide <- df_wide %>% dplyr::rename(`p-value* DSS2` = `Corrected p Value DSS2 Box Cox Transformed and Scaled`,
                              `p-value* IC50` = `Corrected p Value IC50 boxcox sclaed2`)
...
p1_all_metric <- forest(
  df_wide[, c("Experimental Variable", " ", "p-value* DSS2", "p-value* IC50")],              # Main label column
  est = list(df_wide$`Fixed Effect Coefficient IC50 boxcox sclaed2`, df_wide$`Fixed Effect Coefficient DSS3 boxcox sclaed2`, df_wide$`Fixed Effect Coefficient DSS2 Box Cox Transformed and Scaled`, df_wide$`Fixed Effect Coefficient DSS1 boxcox sclaed2`, df_wide$`Fixed Effect Coefficient auc a box cox sclaed2`, df_wide$`Fixed Effect Coefficient AUC boxcox sclaed2`),
  lower = list(df_wide$`Lower IC50 boxcox sclaed2`, df_wide$`Lower DSS3 boxcox sclaed2`, df_wide$`Lower DSS2 Box Cox Transformed and Scaled`, df_wide$`Lower DSS1 boxcox sclaed2`, df_wide$`Lower auc a box cox sclaed2`, df_wide$`Lower AUC boxcox sclaed2`),   # Lower CIs for each category
  upper = list(df_wide$`Upper IC50 boxcox sclaed2`, df_wide$`Upper DSS3 boxcox sclaed2`, df_wide$`Upper DSS2 Box Cox Transformed and Scaled`, df_wide$`Upper DSS1 boxcox sclaed2`, df_wide$`Upper auc a box cox sclaed2`, df_wide$`Upper AUC boxcox sclaed2`),   # Upper CIs for each category
  ci_column = 2,                                      # CI column placement
  ref_line = 0,   
  sizes = 0.5,
  grid = F,
  xlim = c(-2, 2),
  ticks_at = c(-2, -1, 0, 1, 2),
  footnote = "*a LymphoPrepTM gradient centrifugation \n*b Supernatant isolation at 300g 10min, density centrifugation at 400g for 20min without brake, afterwards always 300g 5 min \n*c Bonferroni corrected",
  theme = tm, 
  font.label = list(size = 10, family = "Arial"),
  font.ticks = list(size = 8, family = "Arial"),
  nudge_y = 0.15,
)
...
png('/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/V3/Forest_plot_all_metric.png', height = 10, width = 28, unit = "cm", res = 300) #24.8
grid.newpage()
...
disease_count <- all_response_metrics %>% group_by(Disease.status, drug) %>% summarize(count = n())
...
df_dss2_models_sample <- df_dss2_models_sample %>% mutate(`Biospecimens`= case_when(`Fixed Effect Term` == "TissueBone marrow" ~ "Tissue: Bone Marrow", 
...
df_dss2_models_sample <- df_dss2_models_sample %>% mutate(`n` = case_when(Biospecimens == 'Sample: Frozen' ~ '55-98',
                                                                          Biospecimens == 'Disease Status: Remission' ~ '2-21',
                                                                          Biospecimens == 'Disease Status: Relapse' ~ '50-97',
                                                                          Biospecimens == 'Tissue: Bone Marrow' ~ '270-600',
                                                                          Biospecimens == 'Disease Status: Refractory' ~ '10-145'
                                                                          ))
#df_re_intercept_models$se <- (log(df_re_intercept_models$Upper) - log(df_re_intercept_models$`Fixed Effect Coefficient`))/1.96
p <- forest(df_dss2_models_sample[,c('Biospecimens', ' ', 'p-value', 'n')],
...
plot(p)
p_wh <- get_wh(p)
pdf('/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/V3/Forest_plot_DSS2_biospecimens.pdf',width = p_wh[1], height = 4)
plot(p)
dev.off()

output_width_cm <- 20 #17
output_height_cm <- 16 #13
dpi <- 300  # Resolution in dots per inch
...
# Scale the gtable layout
scaled_p1_all_metric <- gtable::gtable_filter(p, pattern = ".*", trim = TRUE)  # Keep all grobs
scaled_p1_all_metric$widths <- scaled_p1_all_metric$widths * scale_width
scaled_p1_all_metric$heights <- scaled_p1_all_metric$heights * scale_height


png('/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/V3/Forest_plot_blood.png', height = 10, width = 30, unit = "cm", res = 300) #24.8
#10 28
grid.newpage()
grid.draw(scaled_p1_all_metric)
dev.off()
...
spearman_cor_test <- cor.test(new_df$Fresh, new_df$Frozen, method = "spearman")
spearman_cor <- spearman_cor_test$estimate
p_value <- spearman_cor_test$p.value
...
ggsave("/Users/katarinawilloch/Desktop/UiO/Project 1/Figures/V3/Karolinska_fresh_vs_frozen.png", plot = scatterplot_fresh_vs_frozen_karolinska, width = 10.76, height = 8.09, units="cm") #width = 20.76, height = 7.09
...
# Perform a linear model
lm_model <- lm(Frozen ~ Fresh + factor(Patient.num), data = enserink_fresh_vs_frozen)

check_model <- lmer(Frozen ~ Fresh + (1|drug), data = enserink_fresh_vs_frozen)
...
annotation_text <- paste0(
  "R² = ", round(adjusted_r2, 3), 
  "\nFDR = ", format(pval_fdr, scientific = TRUE, digits = 3)
)
